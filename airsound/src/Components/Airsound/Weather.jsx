import { Container } from "react-bootstrap"


const Weather = ()=>{

    return(

        <Container fluid>
        <div className="cardm">
  <div className="cardW">
    <svg className="weather" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"  x="0px" y="0px" width="100px" height="100px" viewBox="0 0 100 100" >  <image id="image0" width="100" height="100" x="0" y="0" href="data:image/png;base64,"></image>
    </svg>
    <div className="main">23 °C</div>
    <div className="mainsub">Dunmore, Ireland</div>

  </div>

  <div className="cardW2">
    <div className="upper">
      <div className="humidity">
        <div className="humiditytext">Humidity 30%</div>
        <svg  viewBox="0 0 30 30" height="30px" width="30px" y="0px" x="0px"  xmlns="http://www.w3.org/2000/svg" id="Layer_1" version="1.1" className="humiditysvg">  
        <image href="data:image/png;base64," y="0" x="0" height="30" width="30" id="image0"></image>
          </svg>
      </div>

      <div className="air">
        <div className="airtext">Wind 8 Km/h</div>
          <svg className="airsvg" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"  x="0px" y="0px" width="30px" height="30px" viewBox="0 0 30 30" >  <image id="image0" width="30" height="30" x="0" y="0" href="data:image/png;base64,"></image>
        </svg>
      </div>
    </div>

    <div className="lower">
      <div className="aqi">
        <svg className="aqisvg" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 20 20" >  <image id="image0" width="20" height="20" x="0" y="0" href="data:image/png;base64,
          MjUrMDA6MDBiirAvAAAAAElFTkSuQmCC"></image>
        </svg>
        <div className="aqitext">AQI 30</div>
      </div>

      <div className="realfeel">
        <svg className="rfsvg" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 20 20" >  <image id="image0" width="20" height="20" x="0" y="0" href="data:image/png;base64,"></image>
        </svg>
        <div className="realfeeltext">Real Feel 21 °C</div>
      </div>

      <div className="pressure">
        <svg className="pressuresvg" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="20px" height="20px" viewBox="0 0 20 20" >  <image id="image0" width="20" height="20" x="0" y="0" href="data:image/png;base64,"></image>
        </svg>
        <div className="pressuretext">Pressure 1012 mbar</div>
      </div>
      <div className="cardW3">
      Healthy
      </div>
    </div>
  </div>
</div>
        </Container>

    )
}

export default Weather